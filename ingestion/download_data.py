...
def download_kaggle_dataset(dataset_slug: str, download_path: str = "data", api: KaggleApi = None) -> None:
    ...
    Args:
        ...
        api (KaggleApi, optional): Pre-authenticated Kaggle API client instance.
    """
    # Ensure the download directory exists
    if not os.path.exists(download_path):
        os.makedirs(download_path)

    # ⚡ Bolt: Use provided API instance or initialize and authenticate a new one
    # This prevents redundant authentication requests when downloading multiple datasets in a loop.
    if api is None:
        api = KaggleApi()
        api.authenticate()
...
if __name__ == "__main__":
    ...
    # ⚡ Bolt: Authenticate once outside the loop to avoid redundant authentication requests
    api = KaggleApi()
    api.authenticate()
