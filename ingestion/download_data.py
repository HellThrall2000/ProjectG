...
def download_kaggle_dataset(dataset_slug: str, download_path: str = "data", api: KaggleApi = None) -> None:
    ...
    Args:
        ...
        api (KaggleApi, optional): An authenticated Kaggle API client instance.
    """
    # Ensure the download directory exists
    if not os.path.exists(download_path):
        os.makedirs(download_path)

    # ⚡ Bolt: Authenticate once by accepting an optional KaggleApi instance to avoid redundant initialization
    if api is None:
        api = KaggleApi()
        api.authenticate()
