...
def download_kaggle_dataset(dataset_slug: str, download_path: str = "data", api: KaggleApi = None) -> None:
    ...
    Args:
        ...
        api (KaggleApi, optional): An authenticated Kaggle API instance. Prevents redundant authentication if provided.
    """
    # Ensure the download directory exists
    if not os.path.exists(download_path):
        os.makedirs(download_path)

    # Performance Optimization: Use injected authenticated API to prevent redundant network auth calls
    if api is None:
        api = KaggleApi()
        api.authenticate()
...
if __name__ == "__main__":
    ...
    # Authenticate once before the loop to avoid authenticating multiple times
