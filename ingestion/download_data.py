...
def download_kaggle_dataset(dataset_slug: str, download_path: str = "data", api: KaggleApi = None) -> None:
    ...
    Args:
        ...
        api (KaggleApi, optional): Pre-authenticated Kaggle API instance for batch processing.
    """
    # Ensure the download directory exists
    if not os.path.exists(download_path):
        os.makedirs(download_path)

    # ⚡ Bolt Optimization: Use injected API client if provided to avoid redundant authentication
    if api is None:
        api = KaggleApi()
        api.authenticate()
...
if __name__ == "__main__":
    ...
    # ⚡ Bolt Optimization: Instantiate and authenticate the Kaggle API once outside the loop
    # to prevent redundant network and disk I/O for authentication token checks.
    batch_api = KaggleApi()
    batch_api.authenticate()

    for ds in datasets:
        download_kaggle_dataset(ds, api=batch_api)
