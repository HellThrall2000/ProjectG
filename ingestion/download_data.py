...
def download_kaggle_dataset(dataset_slug: str, download_path: str = "data", api: KaggleApi = None) -> None:
    ...
    Args:
        ...
        api (KaggleApi, optional): An authenticated KaggleApi instance to prevent redundant authentication overhead.
    """
    # Ensure the download directory exists
    if not os.path.exists(download_path):
        os.makedirs(download_path)

    # ⚡ Bolt Optimization: Use injected, authenticated KaggleApi instance to avoid redundant API auth overhead
    if api is None:
        api = KaggleApi()
        api.authenticate()
...
if __name__ == "__main__":
    ...
    # ⚡ Bolt Optimization: Authenticate once outside the loop and inject into download function
    api = KaggleApi()
    api.authenticate()
