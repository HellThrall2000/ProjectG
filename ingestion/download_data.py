...
def download_kaggle_dataset(dataset_slug: str, download_path: str = "data", api: KaggleApi = None) -> None:
    ...
    Args:
        ...
        api (KaggleApi, optional): An authenticated KaggleApi instance to reuse.
    """
    # Ensure the download directory exists
    if not os.path.exists(download_path):
        os.makedirs(download_path)

    # Use injected API or instantiate and authenticate a new one
    if api is None:
        api = KaggleApi()
        api.authenticate()
...
if __name__ == "__main__":
    ...
    # Authenticate once to avoid redundant network overhead
    api = KaggleApi()
    api.authenticate()
